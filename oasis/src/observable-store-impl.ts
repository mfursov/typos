...
import {FetchFn, NeedUpdateFn, ObservableStore, RefreshMode, skipUpdateCheck} from './observable-store';
...
export class ObservableStoreImpl implements ObservableStore {
  ...
  constructor(storeName: string,
              isBrowser: boolean,
              asyncStoreFactory: (name: string, browser: boolean) => AsyncStore,
              serverState?: TransferState,
              schemaVersion?: number,
              private readonly freezeFn = noFreeze) {
    this.asyncStore$$ = new Promise<AsyncStore>(resolve => {
      const asyncStore = asyncStoreFactory(storeName, isBrowser);
      const t0 = Date.now();
      asyncStore.init(schemaVersion || 0).then(() => {
        if (!serverState) {
          resolve(asyncStore);
        ...
        if (isBrowser) {
          ...
          this.updateDbStateFromServerState(asyncStore, serverStateValue).then(() => {
          ...
          serverState.onSerialize(serverStateKey, () => {
            const kvPairs = asyncStore.snapshot();
   ...
   */
  private async updateDbStateFromServerState(adapter: AsyncStore, serverState: any): Promise<void> {
